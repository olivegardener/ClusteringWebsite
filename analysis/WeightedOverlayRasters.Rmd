...
# Function to create a dark-themed plot for a raster
plot_dark_mode_raster <- function(raster, values, title) {
  num_breaks <- 255
  breaks <- seq(values[1], values[2], length.out = num_breaks)
  levelplot(raster, col.regions = color_ramp(num_breaks), at = breaks, main = title,
            par.settings = list(
              axis.line = list(col = "white"),
              axis.text = list(col = "white"),
              strip.background = list(col = "#6b6b6b"),
              strip.border = list(col = "white"),
              background = list(col = "#6b6b6b"),
              main.text = list(col = "white"),
              col.lab = "white",
              col.main = "white",
              col.sub = "white"
            ))
...
# Extract the minimum and maximum values for Anthro_x_Risk_norm raster
min_val <- global(Anthro_x_Risk_norm, fun = 'min', na.rm = TRUE)[[1]]
max_val <- global(Anthro_x_Risk_norm, fun = 'max', na.rm = TRUE)[[1]]

# Check if min_val and max_val are finite and plot
if (is.finite(min_val) && is.finite(max_val)) {
    plot_dark_mode_raster(Anthro_x_Risk_norm, c(min_val, max_val), "Anthropogenic Risk Index")
} else {
...
# Check if min_val and max_val are finite and plot
if (is.finite(min_val) && is.finite(max_val)) {
    plot_dark_mode_raster(Bio_x_Risk_norm, c(min_val, max_val), "Anthropogenic Risk Index")
} else {
    cat("Raster contains non-finite or NA values only\n")
}
...
# Check if min_val and max_val are finite and plot
if (is.finite(min_val) && is.finite(max_val)) {
    plot_dark_mode_raster(UrbanProbability_norm, c(min_val, max_val), "Anthropogenic Risk Index")
} else {
    cat("Raster contains non-finite or NA values only\n")
}
...
# Check if min_val and max_val are finite and plot
if (is.finite(min_val) && is.finite(max_val)) {
    plot_dark_mode_raster(Hotspot_Stoplight, c(min_val, max_val), "Hotspot Stoplight")
} else {
    cat("Raster contains non-finite or NA values only\n")
}
